...
class EditarMensalidadeDialog(QDialog):
    """Dialog para edição de mensalidade seguindo o padrão do projeto"""
    def __init__(self, parent, dados_mensalidade):
        super().__init__(parent)
        self.dados = dados_mensalidade
        self.setWindowTitle("Editar Mensalidade")
        self.setFixedSize(500, 750)  # Aumentando altura de 600 para 750
        self.setModal(True)
        self.build_ui()
    ...
    def build_ui(self):
        ...
        aluno_info = QLabel(f"Aluno: {self.dados[7]}")
        aluno_info.setAlignment(Qt.AlignCenter)
        aluno_info.setStyleSheet("font-size: 14px; color: #666; margin-bottom: 15px;")
        card_layout.addWidget(aluno_info)
        ...
        venc_label = QLabel("Data de Vencimento:")
        venc_label.setStyleSheet("font-size: 14px; font-weight: bold; color: #333; margin-top: 10px;")
        card_layout.addWidget(venc_label)
        
        from PySide6.QtWidgets import QDateEdit
        from PySide6.QtCore import QDate
        self.data_venc = QDateEdit()
        self.data_venc.setStyleSheet(input_style)
        self.data_venc.setCalendarPopup(True)
        
        # Definir data atual ou existente
        if self.dados[3]:  # data_vencimento
            try:
                from datetime import datetime
                dt = datetime.strptime(self.dados[3], "%Y-%m-%d")
                self.data_venc.setDate(QDate(dt.year, dt.month, dt.day))
            except:
                self.data_venc.setDate(QDate.currentDate())
        else:
            self.data_venc.setDate(QDate.currentDate())
        
        card_layout.addWidget(self.data_venc)
        
        # Campo Plano
        plano_label = QLabel("Plano:")
        plano_label.setStyleSheet("font-size: 14px; font-weight: bold; color: #333; margin-top: 10px;")
        card_layout.addWidget(plano_label)
        ...
        # Campo Status
        status_label = QLabel("Status:")
        status_label.setStyleSheet("font-size: 14px; font-weight: bold; color: #333; margin-top: 10px;")
        card_layout.addWidget(status_label)
        
        self.status = QComboBox()
        self.status.addItems(["PENDENTE", "PAGO"])
        self.status.setStyleSheet(input_style)
        
        # Definir status atual
        status_atual = self.dados[5] if self.dados[5] else "PENDENTE"
        self.status.setCurrentText(status_atual)
    ...
    def salvar(self):
        ...
        try:
            ...
            cur.execute("""
                UPDATE mensalidades 
                SET data_vencimento=?, status=?
                WHERE id=?
            """, (nova_data_venc, novo_status, mensalidade_id))
            
            # Atualizar plano do aluno
            aluno_id = self.dados[1]  # aluno_id
            cur.execute("""
                UPDATE alunos 
                SET plano=?
                WHERE id=?
            ...
            if hasattr(self.parent(), 'limpar_selecoes_cards'):
                self.parent().limpar_selecoes_cards()
            
            show_info(self.parent(), "Sucesso", "Mensalidade atualizada com sucesso!")
            self.accept()
    ...
    def marcar_pago(self, mensalidade_id):
        ...
        try:
            hoje = date.today().isoformat()
            marcar_mensalidade_paga(mensalidade_id, hoje, "Pagamento confirmado via sistema")
            show_info("Pagamento Confirmado", "Mensalidade marcada como paga com sucesso!")
            self.load()
        except Exception as e:
            show_error(self, "Erro ao confirmar pagamento", f"Erro: {str(e)}")
